...
describe("unsupported SDK ABI direct lifecycle calls", () => {
  test("resume, retry, rewind, and rerun surface the ABI error before mutating run state", async () => {
    ...
    await expect(kernel.resume("r_resume")).rejects.toThrow(/requires workflow SDK ABI 4/);
    await expect(kernel.retry("r_retry")).rejects.toThrow(/requires workflow SDK ABI 4/);
    await expect(kernel.rewind("r_rewind", "keep")).rejects.toThrow(/requires workflow SDK ABI 4/);
    await expect(kernel.rerun("r_rerun")).rejects.toThrow(/requires workflow SDK ABI 4/);
